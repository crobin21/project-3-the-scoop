...
function createComment(url,request) {
  const incomingComment = request.body && request.body.comment;
  const response = {};

  if (incomingComment && incomingComment.body && database.users[incomingComment.username] && database.articles[incomingComment.articleId]) {
    const comment = {
      id: database.nextCommentId++,
      body: incomingComment.body,
      username: incomingComment.username,
      articleId: incomingComment.articleId,
      upvotedBy: [],
      downvotedBy: []
    };
...
function getComment(url, request) {
  ...
  if (comment) {
    comment.body = comment.commentIds.map(
      commentId => database.comments[commentId]);
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    database.users[savedComment.username].commentIds.splice(savedComment.id-1,1);
    database.articles[savedComment.articleId].commentIds.splice(savedComment.id-1,1);
    response.status = 204;
